...
/**
 * Reusable card grid: search box + optional dropdown filter(s) + a grid of
 * cards + pagination. Same search/filter/pagination brain as DataTable,
 * but renders a card per item instead of a table row.
 *
 * @param {object[]} data - Full item data (unfiltered).
 * @param {(item: object) => React.ReactNode} renderCard - Renders a single card.
 * @param {(item: object) => string|number} itemKey - Returns a unique key for an item.
 * @param {string[]} [searchKeys] - Item fields to match against the search box.
 ...
 * @param {string} [emptyMessage="No results match this search."]
 * @param {string} [gridClassName] - Override the default responsive grid classes.
 * @param {boolean} [showResultsCount=true]
 */
const CardGrid = ({
  data,
  renderCard,
  ...
  gridClassName = "grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3",
  ...
  const filtered = useMemo(() => {
    return data.filter((item) => {
      ...
      const matchesFilters = filters.every(({ field, options }) => {
        ...
        return active === options[0] || item[field] === active;
      });
  ...
  return (
    <div>
      ...
      {/* Card grid */}
      <div className={`mt-5 ${gridClassName}`}>
        {pagedItems.map((item) => (
